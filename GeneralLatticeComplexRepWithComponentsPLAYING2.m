function [f1,f2]=GeneralLatticeComplexRepWithComponentsPLAYING2()
...
%             con = contourf(X,Y,(totalFromCompSum),N);
            ...
            iPts = GuideLineBoundaryIntersectionPoints(deltaKs(jj,:),deltaPhis(jj),xMin,xMax,yMin,yMax);
...
function intersectionPoints = GuideLineBoundaryIntersectionPoints(dKvec,dPh,xMin,xMax,yMin,yMax)
    % This function solves for where the cosine terms in the potential are
    % maximized.  I.e. it solves for lines along which 
    % dKvec.*[x,y] + dPh = n*2*pi
    ...
    if sum(abs(dKvec))==0
        ...
        if abs(dKvec(2))>abs(dKvec(1))
            ...
            r0 = [0,dPh/dKvec(2)]; % r0 is just a point on the line where dKvec.*[x,y] + dPh = 0
        else
            r0 = [dPh/dKvec(1),0];
        end
        ...
        for n = nMin:nMax
            ...
            xMinIntersect = (-(dKvec(1)/dKvec(2))*xMin + (dPh + n*2*pi)/dKvec(2));
            ...
            xMaxIntersect = (-(dKvec(1)/dKvec(2))*xMax + (dPh + n*2*pi)/dKvec(2));
            ...
            yMinIntersect = (-(dKvec(2)/dKvec(1))*yMin + (dPh + n*2*pi)/dKvec(1));
            ...
            yMaxIntersect = (-(dKvec(2)/dKvec(1))*yMax + (dPh + n*2*pi)/dKvec(1));
